...
Every -01 Foundation + CRUD story must include these four ACs (in this order, before the grid defaults AC). Input grid stories include a fifth mandatory AC for new record creation (§6).
...
### AC — New Record Creation (§6) — Input Grids Only

For input grids (inline-editable), include this AC to cover the empty editable row and Add Row button. Omit for read-only/inquiry screens.
...
<p>Reference: See <a href="https://dev.azure.com/QuorumSoftware/Quorum/_wiki/wikis/Quorum.wiki/11638/?wikiVersion=GBwikiMaster&amp;pagePath=/Quorum/North%20America/Upstream/myQ%20Modernization/EAM%252DScreen%252DMigration%252DFunctionality%252DReference&amp;pageId=11638&amp;anchor=6.-add-row-%25E2%2580%2594-new-record-creation">Section 6 — Add Row — New Record Creation</a> in the EAM Functionality Reference Wiki.</p>
```

> **Rules:** (1) This AC covers the grid's row creation mechanism — the empty editable row and the Add Row button. (2) How the row's fields get populated (via pick lists, typed entry, etc.) belongs in a separate screen-specific AC. (3) Omit this AC entirely for read-only grids.
...
### AC — Pagination & Grid Defaults (§3 + §21/§22/§23/§24/§25) — LAST FUNCTIONAL AC

```html
<p><strong>AC[N] — Pagination and grid interaction defaults (export, import, highlight, copy, bulk view/edit):</strong></p>
<ul>
  ...
  <li>Then
    <ul>
      ...
      <li>Import from Excel button allows bulk data upload from a formatted Excel template into the grid (input grids only — omit this bullet for read-only grids)</li>
