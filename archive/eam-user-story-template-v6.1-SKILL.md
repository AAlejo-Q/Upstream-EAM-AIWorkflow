> **ARCHIVED 2026-07-08.** This is the retired v6.1 EAM User Story Authoring Template, superseded by `project-files/eam-story-core.md` and `project-files/eam-ac-library.md`. It was never actually loaded in practice — `eam-project-instructions.md` explicitly overrides it ("Do NOT load or follow `/mnt/skills/user/eam-user-story-template/SKILL.md` for any EAM story work"). Kept here for historical reference only, including its now-superseded "required fields highlighted" language (retired in eam-ac-library.md v1.6). Do not use for current story authoring.
...
JE020-05: [Special Functionality Name]
JE020-06: Testing, QA Parity & Classic Screen Sign-Off
...
| -04 | PPN | `[ScreenID]-04: PPN Triggers and Creation` | Screen generates Prior Period Notification records |
| -05+ | Other Special | `[ScreenID]-05: [Special Functionality Name]` | Mass Changes, Batch Integration, Child Grids, Advanced Rules, or any other distinct special capability |

When a slot is not applicable, skip it and continue sequential numbering with the next applicable story.

> **Audit History is excluded from per-screen story creation.** Screens with audit history capability will be addressed as a batch in a dedicated initiative. Do not create Audit History stories as part of individual screen story maps.
...
| With Notes | -01, -02, -03 Notes, -[N] QA | -Security, -API |
| With Notes + PPN | -01, -02, -03 Notes, -04 PPN, -[N] QA | -Security, -API |
| Full special stack | -01, -02, -03 Notes, -04 PPN, -05+ Other, -[N] QA | -Security, -API |
...
- **Special stories follow fixed order** — Notes (-03), PPN (-04), Other (-05+); omit slots that don't apply, continue numbering sequentially. Audit History is excluded from per-screen story creation (deferred to a batch initiative)
...
| Notes (-03), PPN (-04), Special (-05+) | `eam-phase-special` |
...
Multiple tags are separated by semicolons when needed (e.g., `"eam-phase-ui-crud; some-other-tag"`). Do NOT add a standalone `eam` tag — the `eam-phase-*` tags are sufficient.
