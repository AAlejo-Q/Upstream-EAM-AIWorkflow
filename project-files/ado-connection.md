...
- **Project**: `QuorumSoftware`
- **Expand**: Use `expand: relations` when you need parent/child hierarchy or
  predecessor/successor dependency links. Use `expand: all` for full field data
  without relations. These are mutually exclusive — choose based on what you need.
  When auditing link chains, always use `expand: relations`.
- **Project for link operations**: When calling `wit_work_items_link` for EAM work
  items, use `project: "Quorum"` — not `"QuorumSoftware"`. EAM stories live in the
  `Quorum` project within the QuorumSoftware org. Using the wrong project ID on link
  calls results in a Not Found error even when the work item IDs are correct.
